...
class ChessGame: 
    ...
    def run(self):
        running = True
        #----------------------------------Initialization-------------------------------
        current_player = 'black'       # start from black
        ...
        while running:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:  # mouse clicked event
                    ...
                    game_logic.handle_click(clicked_position)
                    game_render.render_promote_button(pygame.MOUSEBUTTONDOWN)
...
class Initial_interface:
    ...
    def draw_start_button(self):       # 始まりボタン
...
class Game_renderer:
    def __init__(self, window, width, height, cell_size, user_space, label_width, game_logic):
        ...
        self.buttons = None
        self.UI_y1 = self.user_space/2
        self.UI_y2 = self.height - self.user_space // 2
    ...
    def render_timer(self):               # timer
        ...
        timer_text1 = pygame.font.Font(None, 36).render(f"Time Left: {int(self.game_logic.time_left)}",True,(255, 255, 255))
        timer_text2 = pygame.font.Font(None, 36).render(f"Time Left: {int(self.game_logic.time_left)}",True,(0, 0, 0))
    ...
    def render_regret_button(self, event_type):
        button_width = 60
        button_height = 40
    ...
    def render_check(self):
        ...
        check_text1 = pygame.font.Font(None, 36).render(f"check!",True,(255, 255, 255))
        check_text2 = pygame.font.Font(None, 36).render(f"check!",True,(0, 0, 0))
        text_rect1 = check_text1.get_rect(center=(x1, y1))
        text_rect2 = check_text2.get_rect(center=(x2, y2))
        if self.game_logic.check[0] == True:
            self.window.blit(check_text1, text_rect1)
        if self.game_logic.check[1] == True:
            self.window.blit(check_text2, text_rect2)
    ...
    def render_promote_button(self, event_type):        # pawn promotion button
        if self.game_logic.promote_piece is not None:
            ...
            button_height = 40
            button_gap = 20
            if self.game_logic.promote_piece.color == 'black':
    ...
    def click(self, game_logic):
        try:
            ...
            if len(game_logic.history) > 1:                               # keep initial state
                ...
                previous_chessboard, previous_round, previous_player, previous_promote_piece, previous_passant_piece, previous_is_pawn_event, previous_check, previous_king = copy.deepcopy(game_logic.history[-1])  # 一つ前の状態を得る
                game_logic.chessboard = previous_chessboard
                game_logic.round = previous_round
...
class Game_logic:
    def __init__(self, selected_piece, current_player, time_left, turn_time):
        ...
        self.round = 0                          # game round (number of turn)
    ...
    def initialize_pieces(self):     # initial position for pieces
        self.chessboard[(0, 0)] = Rook('white', (0, 0), self.chessboard)
        self.chessboard[(0, 1)] = Knight('white', (0, 1), self.chessboard)
        self.chessboard[(0, 2)] = Bishop('white', (0, 2), self.chessboard)
        self.chessboard[(0, 3)] = King('white', (0, 3), self.chessboard)
        self.chessboard[(0, 4)] = Queen('white', (0, 4), self.chessboard)
        self.chessboard[(0, 5)] = Bishop('white', (0, 5), self.chessboard)
        self.chessboard[(0, 6)] = Knight('white', (0, 6), self.chessboard)
        self.chessboard[(0, 7)] = Rook('white', (0, 7), self.chessboard)

        self.chessboard[(7, 0)] = Rook('black', (7, 0), self.chessboard)
        self.chessboard[(7, 1)] = Knight('black', (7, 1), self.chessboard)
        self.chessboard[(7, 2)] = Bishop('black', (7, 2), self.chessboard)
        self.chessboard[(7, 3)] = King('black', (7, 3), self.chessboard)
        self.chessboard[(7, 4)] = Queen('black', (7, 4), self.chessboard)
        self.chessboard[(7, 5)] = Bishop('black', (7, 5), self.chessboard)
        self.chessboard[(7, 6)] = Knight('black', (7, 6), self.chessboard)
        self.chessboard[(7, 7)] = Rook('black', (7, 7), self.chessboard)

        for col in range(8):
            self.chessboard[(1, col)] = Pawn('white', (1, col), self.chessboard)
            self.chessboard[(6, col)] = Pawn('black', (6, col), self.chessboard)

    def into_history_stack(self):               # always keep the initial state and push state per motion
        current_state = (copy.deepcopy(self.chessboard), self.round, self.current_player, copy.deepcopy(self.promote_piece), copy.deepcopy(self.passant_piece), self.is_pawn_event, self.check, self.king)
    ...
    def if_pawn_time(self, color, event):
        if event == 'promotion':
    ...
    def move_piece(self, clicked_position):
        self.chessboard[self.selected_piece.position] = None
        self.selected_piece.position = clicked_position
        if self.king['black'] == self.chessboard[clicked_position]:
            ...
            self.check[1] = None
        self.chessboard[clicked_position] = self.selected_piece
        ...
        if 0 <= clicked_position[0] < 8 and 0 <= clicked_position[1] < 8:
            ...
            if self.is_pawn_event == False and self.selected_piece is None:  # no selected -> select
                ...
                # clicked the cell which on selected piece's movable route 
                    ...
                    self.move_piece(clicked_position)       # move
                    self.update_state()
                    if isinstance(self.selected_piece, Pawn):
                        pawn = self.selected_piece
                        if pawn.first_move == True: # pawn first step
                            pawn.first_move = False
                            if False:       # pawn Enpassant
                                pass 
                        if pawn.color == 'black' and pawn.position[0] == 0 or pawn.color == 'white' and pawn.position[0] == 7: # pawn promotion
                            self.promote_piece = pawn
                            self.is_pawn_event = True
                            self.if_pawn_time(self.promote_piece.color, 'promotion')
                        else:
                            self.reverse_player()
                    else:
                        self.reverse_player()
    ...
    def update_real_time_king(self):          # get real time king (black_king, white_king)
        for row in range(8):
            for col in range(8):
                piece = self.chessboard[(row, col)]
                if self.king['black'] and self.king['white']:
                    break
                if  piece and isinstance(piece, King):
                    self.king[piece.color] = piece
    ...
    def if_check(self):
        ...
        elif not self.king['black']:
            pass        # black loss
        elif not self.king['white']:
            pass        # white loss
                    ...
                    for x in range(8):
                        for y in range(8):
                            if piece.movable_list[x][y] == True:     # movable position
                                if self.chessboard[(x, y)] == self.king[player]:        # if king on the movable position
                                    return True
        return False                  # for all pieces, can't be checked
    
    def space(self):
        self.space_e = True
        self.if_check()
        #print(self.selected_piece.movable_list)
